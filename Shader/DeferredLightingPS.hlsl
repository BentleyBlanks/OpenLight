...
float4 DeferredLightingPSMain(StandardQuadPSInput pin):SV_TARGET
{
    ...
    {
        normalize(float3(10,1,1)),
        normalize(float3(1,1,1)),
        normalize(float3(0,0,1))
    };
    float3 diffuse = diffuseMap.Sample(sam,pin.texcoord).xyz;
    float3 normal = normalMap.Sample(sam,pin.texcoord).xyz;
    ...
    for(int i = 0;i<3;++i)
    {
        ...
        color += diffuse * pointColors[i] * 0.1 + factor * diffuse * pointColors[i];
    }
