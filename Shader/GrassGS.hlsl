...
//    float3 right = float3(1,0,0);
    ...
    float  windAmplitude = grassSize.z;
    ...
    for(int i=1;i<6;++i)
    {
        ...
        v[baseIndex + 0].positionW += right * windAmplitude * i * windOffset;
        ...
        v[baseIndex + 1].positionW += right * windAmplitude * i * windOffset;
...
//     windStride = 5.f;
    ...
    float3 windDir = normalize(float3(windTime.y,0.f,windTime.z));
    float  windAmplitude = grassSize.z;
...
//        float windOffset = 1.f;
        float windOffset = perlinNoiseMap.SampleLevel(samPoint,gin[0].texcoord,0.f).y;

        
        int baseIndex              = i * 2;
        v[baseIndex + 0].positionW = v[0].positionW + i * float3(0.f,halfSize.y,0.f);
        v[baseIndex + 0].positionW += windDir * windAmplitude * i * windStride * windOffset;
        v[baseIndex + 0].normalW   = look;
        v[baseIndex + 0].tangentW  = right;
